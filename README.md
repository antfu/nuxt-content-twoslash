...
### Nuxt v4 Type Contexts (Project References)

Nuxt v4 uses [TypeScript project references](https://nuxt.com/docs/4.x/guide/concepts/typescript#project-references), which split your project into separate type contexts: `app`, `server`, `node`, and `shared`. Each context has its own `tsconfig` with different compiler options, path mappings, and available auto-imports.
