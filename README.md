...
### Nuxt v4+ Project-Based TypeScript Configuration

Nuxt v4 uses [project references](https://nuxt.com/docs/4.x/guide/concepts/typescript#project-references) with multiple `tsconfig` files for different contexts:

- **tsconfig.app.json** - For application code (components, pages, composables, plugins)
- **tsconfig.node.json** - For build-time/Node.js code (nuxt.config.ts, module files)
- **tsconfig.server.json** - For server/API routes
- **tsconfig.shared.json** - Shared configuration

This module automatically detects and uses the appropriate TypeScript configuration based on the filename hint in your code blocks.

#### Filename-Based Detection

Specify a filename in your code block to automatically use the correct TypeScript configuration:

````md
```ts twoslash [server/api/hello.ts]
// Automatically uses tsconfig.server.json
...
// Automatically uses tsconfig.node.json
export default defineNuxtConfig({
  modules: ['@nuxt/content']
})
```

```ts twoslash [components/MyButton.vue]
// Automatically uses tsconfig.app.json
const count = ref(0)
```
````
...
You can also explicitly specify which TypeScript configuration to use with `context:` metadata:
...
For Nuxt v3 projects or when project-based configs don't exist, the module automatically falls back to using `.nuxt/tsconfig.json`.
